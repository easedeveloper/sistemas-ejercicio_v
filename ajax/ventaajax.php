...
switch ($_GET["op"]) {
    case 'guardaryeditar':
        if (empty($idventa)) {
            $rspta = $ventaM->insertar($idcliente,$idusuario,$tipo_comprobante,$serie_comprobante,
    ...
    case 'anular':
        $rspta = $ventaM->anular($idventa);
        echo $rspta ? "Venta anulada" : "Venta no se puede anular";
    break;

    case 'mostrar':
        $rspta = $ventaM->mostrar($idventa);
        echo json_encode($rspta);
    break;

    case 'listarDetalle':
        $id = $_GET["id"];
        $rspta = $ventaM->listarDetalle($id);
		$total=0;
    ...
    case 'listarArticuloVenta':
        require_once '../modelos/Articulo.php';
        $articuloM = new Articulo();
        $rspta = $articuloM->listarActivosVenta();
        $data = Array();
