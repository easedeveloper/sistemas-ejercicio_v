...
Class Usuario
{
    ...
    public function insetar($nombre,$tipo_documento,$num_documento,$direccion,$telefono,$email,$cargo,$login,$clave,$imagen,$permisos)
    {
        ...
        /* me va a devolver el id del usuario registrado */
        $idusuarionew = ejecutarConsulta_retornarID($sql);

        $num_elementos=0; //no tengo ni un permiso asignado
        $sw=true; //una bandera para indicar que no tengo ningun error

        //count(), cuenta los elementos en el array o alguna cosa en el objeto
        while ($num_elementos < count($permisos)) 
        {
            $sql_detalle = "INSERT INTO usuario_permiso(idusuario, idpermiso)
            VALUES('$idusuarionew','$permisos[$num_elementos]')";//$permisos[$num_elementos], numero de elemtos guardados
    ...
    public function verificar($logina, $clavehash)
    {
        ...
        FROM usuario WHERE login='$logina' AND clave='$clavehash' AND condicion='1'";
        return ejecutarConsulta($sql);
    }
